...
const DialogContent = React.forwardRef(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      <DialogClose className="absolute right-4 top-4 rounded-lg p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-ring">
        <X className="h-4 w-4" />
      </DialogClose>
    </DialogPrimitive.Content>
  </DialogPortal>
...
const DialogFooter = ({ className, ...props }) => (
  <div className={cn('flex items-center justify-end gap-2 p-6 pt-4 border-t border-slate-800', className)} {...props} />
)
